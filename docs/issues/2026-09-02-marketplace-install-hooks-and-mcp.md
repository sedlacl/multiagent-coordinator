...
- `mcp-cursor.json` (referenced from `.cursor-plugin/plugin.json`) anchors the script with `${CURSOR_PLUGIN_ROOT}`; `mcp-claude.json` uses `${CLAUDE_PLUGIN_ROOT}` and the root `mcp.json` uses the Agent Plugins `${PLUGIN_ROOT}`.

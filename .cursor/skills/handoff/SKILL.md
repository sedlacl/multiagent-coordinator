...
1. Read current state with MCP `get_handoff`. If MCP is missing, read the file. Empty file is fine.
...
3. Write with MCP `write_handoff`. If MCP is missing, overwrite the file. Max 8000 characters.
4. Reply in the user's language: one sentence that it was replaced, plus Goal and Next actions only.
